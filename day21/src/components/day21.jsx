		...
		<div className="card w-50 p-2 my-2" style={{ width: "18rem" }}>
			<div className="card-body">
				...
				<div className="d-flex ">
					...
					<button className="btn mx-3 btn-primary" onClick={() => setIsEditing(!isEditing)}>
						{isEditing ? "Done" : "Edit"}
					</button>
				</div>
			</div>
		...
		<div className="container d-flex flex-column align-items-center">
			...
			{items.length === 0 ? (
				...
				items.map((item, index) => (
					<RenderItem item={item} key={index} index={index} updateItem={updateItem} removeItem={removeItem} />
				))
			)}
		</div>
	);
